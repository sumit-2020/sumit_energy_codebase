...
/*
    for (uint pid = 0; pid < std::extent<decltype(pattern0)>::value; pid++)
    {
        ...
        for(int tras = 13; tras >= 1; tras-=2)
        //for(uint tras = 1; tras >= 1; tras--)
        {
            printf("tRAS: %u \n", tras);
            ...
            modified_tras_test2(fpga,wait_time,tras, ras_f, ras_cnt_f, pattern0[pid]) ;
            ras_cnt_f.close();
        }
        ras_f.close();
    }
    ...
    for (uint pid = 0; pid < std::extent<decltype(pattern0)>::value; pid++)
    {
        ...
        for(uint twr = 6; twr >= 1; twr--)
        {
            ...
            test_twr_col_order(fpga,wait_time,twr, wr_f, wr_cnt_f, pattern0[pid]) ;
            wr_cnt_f.close();
        }
        wr_f.close();
    }
