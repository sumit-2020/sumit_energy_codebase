...
void writeAllPattern(fpga_t *fpga, uint pattern, CmdQueue*& cq, uint idle)
{
    ...
    for (uint bank = 0; bank < NUM_BANKS; bank++) {
        for (uint row = 0; row < NUM_ROWS; row++) {
            printf("(%u,%u)\n", bank, row);
            writeRow(fpga, row, bank, pattern, cq, NUM_COLS);
            if (!idle) {
...
void closedRowReadMiss(fpga_t *fpga, CmdQueue*& cq, uint pattern)
{
	...
	//printf("bank, row: %u %u\n", bank, row);
	//refresh(fpga, cq, row % NUM_BANKS, 0, DEF_RETENTION, 0);
	readAndCompareRow(fpga, row, bank, pattern, cq, NUM_COLS , 0 , false);
      }
    }
}
...
{
    uint startBank = 0;
    uint endBank = NUM_BANKS-1;
    uint startRow = 0;
    uint endRow = NUM_ROWS-1;
    uint numRws = 10000;
    uint policy = CLOSE;
    ...
    switch (test_name) {
        ...
        case ACT_PRE_TEST:
            ...
            break;
        case IDD0_TEST:
            // Test 6: ACT-PRE only one single row repeatedly
            idd0Test(fpga, cq, pattern, NUM_BANKS/2, NUM_ROWS/2, numRws, num_cols);
        default:
            printf("Before default\n");
            idleTest();
    }
...
void idd0Test(fpga_t *fpga, CmdQueue*& cq, uint pattern, uint bank, uint row, 
    ...
    for (uint iter = 0 ; iter < numRws ; iter++ ){
        cq->insert(genRowCMD(0, bank, MC_CMD::PRE));    
        cq->insert(genWaitCMD(DEF_TRP));                
        cq->insert(genRowCMD(row, bank, MC_CMD::ACT));
        cq->insert(genWaitCMD(DEF_TRCD));
...
void test(fpga_t* fpga, uint pattern, CmdQueue*& cq)
{
    ...
    // NEW TESTS
    doTest(fpga, cq, IDD0_TEST, pattern);
...
void baselineTest(fpga_t *fpga)
{
  ...
  //writeRow(fpga, row, bank, pattern, cq, 1024);
  //readAndCompareRow(fpga,row,bank, pattern, cq, 1024);
