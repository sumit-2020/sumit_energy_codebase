...
void readAndCompareRow(fpga_t* fpga, const uint row, const uint bank, const uint8_t pattern, CmdQueue*& cq ){
	...
	for(int i = 0; i < NUM_COLS; i+=8){ //we receive a single burst at two times (32 bytes each)
        ...
        printf("Returned from fpga_recv\n");
...
void setRefreshConfig(fpga_t* fpga, uint trefi, uint trfc){
	...
	cq->insert(genRefConfigCMD(trfc, MC_CMD::SET_TRFC));
	cq->insert(genRefConfigCMD(trefi, MC_CMD::SET_TREFI));
...
int main(int argc, char* argv[]){
	...
	//uint trfc = 260; //default trfc for 8Gb device
