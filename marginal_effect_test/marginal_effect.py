...
def get_test_args():
    test_args = {
        "test_name" : ["fixedtime_actnturnbuswrpre"],
        "row"       : [0],
        "n"         : [0,2,4,8,16,32,48,64,80,96,112,128,144,160,176,192,208,224,240],
    ...
    test_args["n_max"] = max(test_args["n"])
...
def __main__():
    ...
    data_dict["raw_data"] = []
    trsh_dict["raw_data"] = []
    data_dict["avg_voltage"] = []
    trsh_dict["avg_voltage"] = []

    meter = init_meter_connection(args.dimm == "sandbox")
            ...
            for r in test_args["row"]:
                for p in test_args["pattern"] :
                    for n in test_args["n"]:
                        test = {"test_name":t, "bank":b, "row":r, "pattern": p, "n":n, "n_max":test_args["n_max"]}
                        averages = []
                        garbages = []
                        attempt  = 0
                        remove_max = True
                        log = "["+str_timestamp()+"] Running: "
                        log += "./bin/iddtest " + " " + args.dimm + " " + t + " "
                        log += "0x{:02X}".format(p) + " " + str(b) + " " + str(r) + " "
                        log += str(n)+" "+str(test_args["n_max"])
                        print log

                        while len(averages) < args.iter and attempt <= args.iter * 3:
                            attempt += 1
                            # Measure a new set of values
                            run_binary(attempt, args.dimm, t, bank=b, row=r, pattern=p, num_ops=n, max_num_ops=test_args["n_max"])
                            sleep(0.01)
                            data_arr = get_measurements(meter,args.dimm == "sandbox")
...
def append_data(data_dict, test, data_arr):
    ...
    data_dict["n_max"].append(test["n_max"])
    data_dict["raw_data"].append(raw_data)
    data_dict["avg_voltage"].append(stats.mean(data_arr))
    return data_dict

def run_binary(i, dimm, test_name, bank=0, row=0, pattern=0x00, num_ops=0, max_num_ops=0):
    args = [dimm,test_name,str(pattern),str(bank),str(row),str(num_ops),str(max_num_ops)]
