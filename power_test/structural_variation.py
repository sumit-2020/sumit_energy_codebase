...
def main():
    ...
    banks = range(4,8)
    data_patterns = [0x00, 0xaa, 0xff]
    if args.temp == 20:
        test_names = ["idd0", "idd1"]
    elif args.temp == 70:
        test_names = ["idd0"]
    else :
        print "WTF: unknown temperature!"
    ...
    for bank in banks:
        if args.temp == 20:
            if bank == 4:
                rows = range(808,1024)
            elif bank == 6 or bank == 7:
                rows = range(512,1024)
            else :
                rows = range(342,1024)
        elif args.temp == 70:
            rows = get_sample_rows(args.dimm, bank)
        else :
            print "WTF: unknown temperature!"

        print args.dimm + " bank " + str(bank) + ":"
        print rows
